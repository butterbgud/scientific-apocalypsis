  ...
  return (
    <main className="game-shell">
      <header className="game-header">
        ...
        <div className="header-actions"><div className="header-status"><span className="round-status-top">{notice}</span>{player && player.sentThisRound >= 1 && <button className="primary-button next-round-top" onClick={nextRound}>{ui.nextRound}</button>}</div><div className="header-buttons"><span className="turn-pill"><Crown size={15} /> {ui.first}</span><button className={`history-toggle ${historyVisible ? "active" : ""}`} onClick={() => setHistoryVisible(!historyVisible)} aria-label={historyVisible ? "Hide history" : "Show history"}>H</button><button className="ghost-button" onClick={() => setScreen("lobby")}>{ui.exit}</button></div></div>
      </header>
      ...
      <div className="board-layout">
        ...
        <section className="board-center">
          ...
          <div className="board-footer"><div><span className="eyebrow">{ui.selected}</span><p>{selectedTaskData ? selectedTaskData.title : ui.noTask}</p></div><div className="footer-actions"><div className="disaster-chip"><HeartPulse size={16} /> {ui.disasters}</div></div></div>
          {historyVisible && <div className="history-panel"><div className="history-heading"><span className="eyebrow">{ui.history}</span><span>{game?.history.length ?? 0} {ui.events}</span></div>{(game?.history ?? []).slice(-5).reverse().map((event, index) => <div className="history-event" key={`${event.type}-${index}`}><span className="history-dot" /><div><strong>{event.message}</strong>{event.data?.dice ? <small>{english ? "Dice" : "Кубик"}: {String(event.data.dice)} · {english ? "Result" : "Результат"}: {String(event.data.score)} / {String(event.data.difficulty)}</small> : null}</div></div>)}</div>}
        </section>
